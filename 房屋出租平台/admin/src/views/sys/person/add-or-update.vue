<template>
  ...
  >
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="100px" style="width:95%;" @keyup.enter.native="dataFormSubmit()">
      <el-row>
        <el-col :span="24">
          <el-form-item label="物业公司" prop="agentId">
            <el-select v-model="dataForm.agentId" placeholder="请选择物业公司" filterable style="width: 100%" @change="getcommunityList($event)">
              <el-option v-for="agent in agentList" :key="agent.agentId" :label="'[' + agent.agentNo + ']' + agent.agentName" :value="agent.agentId" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="24">
...
export default {
  ...
  methods: {
      ...
      getInfo(id).then(res => {
        if (res && res.code === 200) {
          ...
          if (id > 0) {
            ...
            this.dataForm.agentId = res.data.person.agentId
    ...
    getcommunityList(agentId) {
      this.dataForm.communityId = ''
      getCommunityList(agentId).then(res => {
        if (res.code === 200) {
