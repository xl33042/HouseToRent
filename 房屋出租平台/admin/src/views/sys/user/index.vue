  ...
  <div class="app-container">
    <el-form :inline="true" :model="listQuery" @keyup.enter.native="initData()">
      ...
      <el-form-item label="用户类别">
        <el-select v-model="listQuery.userType" filterable placeholder="请选择" clearable>
          <el-option
      ...
      <el-row>
        <el-col :span="24">
          ...
          <el-form-item>
            <el-button v-has="'sys:user:edit'" type="primary" icon="el-icon-edit" :disabled="ids.length <= 0" @click="addOrUpdateHandle()">编辑</el-button>
          </el-form-item>
          <el-form-item>
            <el-button v-has="'sys:user:del'" type="primary" icon="el-icon-delete" :disabled="ids.length <= 0" @click="deleteHandle()">删除</el-button>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
    ...
    <el-table ref="topictable" v-loading="loading" :height="tableHeight" :row-style="rowStyle" size="mini" border :data="tableData" @selection-change="selectionChangeHandle">
      ...
      <el-table-column prop="userType" label="账户类别" width="120" align="center">
        <template slot-scope="scope">
          <p v-if="scope.row.userType === 1">系统管理员</p>
          <p v-else-if="scope.row.userType === 2">单位管理员</p>
        </template>
      </el-table-column>
      <el-table-column prop="agentName" label="所属机构" show-overflow-tooltip align="center" />
...
<script>
import { getDataList, del, exportExcel, getAllAgent } from '@/api/sys/user'
